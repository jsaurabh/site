...
export default function WritingPage() {
  return (
    <div id="top"><SiteHeader /><main>
      ...
      <section className="section page-shell"><div className="writing-list writing-list-page">{writing.map((item, index) => <article key={item.title}><div><span>{item.date}</span><span>{item.type}</span></div><span className="writing-number">0{index + 1}</span><h2><Link href={`/articles/${item.slug}`}>{item.title}</Link></h2><p>{item.dek}</p><span className="row-arrow">→</span></article>)}</div></section>
